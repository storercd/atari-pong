...
class AtariGymWrapper():
    ...
    @staticmethod
    def wrap(env):
        # call order here is important due to the specific nature of function
        #  overrides.  For FIRE-start games with multiple lives, we must fire
        #  after the soft reset, not the hard one.
        env = NoopResetEnv(env, noop_max=30)
        env = EpisodicLifeEnv(env)
        if 'FIRE' in env.unwrapped.get_action_meanings():
            env = FireResetEnv(env)
        # env = ChannelsFirstImageShape(env) # TODO: analyxze and reinstate
        # env = FrameStack(env, 4) # TODO: analyxze and reinstate
